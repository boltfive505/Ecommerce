...
using Egate_Ecommerce.Converters;

namespace Egate_Ecommerce.Objects.Tutorials
{
    public class TutorialVideoViewModel : INotifyPropertyChanged, Egate_Ecommerce.Objects.Calendar.IPeriodGetter
    {
        private HtmlToPlainTextConverter htmlToText = new HtmlToPlainTextConverter();
        ...
        public string ThumbnailFullPath
        {
            get
            {
                if (!string.IsNullOrEmpty(ThumbnailFile))
                    return FileHelper.GetFile(ThumbnailFile, Classes.TutorialsHelper.TUTORIAL_THUMBNAIL_FOLDER);
                return string.Empty;
            }
        }
